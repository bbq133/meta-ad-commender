...
export const ActionItemsTab = forwardRef<ActionItemsTabRef, ActionItemsTabProps>(({
    ...
    const [budgetAdviceFilter, setBudgetAdviceFilter] = useState<'all' | 'stop' | 'reduce' | 'watch'>('all'); // New Budget Advice Filter
    ...
    const filteredBlResult = useMemo(() => {
        ...
        // Budget Advice Filter logic removed from global state.
        // It will be applied locally in the Campaign AI Summary render section.
    ...
    }, [blResult, blRemovedIds, blFilterLevel, blSearchText, blBusinessLineFilter, blPriorityFilter, budgetAdviceFilter]);
                    ...
                    {activeSubTab === 'businessLine' && filteredBlResult && (
                        <>
                            ...
                            <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-2xl border-2 border-indigo-200 shadow-xl overflow-hidden">
                                ...
                                {/* Campaign 详细总结表格部分 */}
                                {blResult && blResult.campaigns.length > 0 && (
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                <h4 className="text-base font-black text-slate-900 flex items-center gap-2">
                                                    <span>📊</span>
                                                    <span>Campaign 层级AI总结</span>
                                                </h4>

                                                {/* 预算建议筛选组件 */}
                                                <div className="flex items-center bg-white rounded-lg p-1 border border-slate-200" onClick={(e) => e.stopPropagation()}>
                                                    {[
                                                        { value: 'all', label: '全部' },
                                                        { value: 'stop', label: '🔴 立即关停' },
                                                        { value: 'reduce', label: '🟡 下调预算' },
                                                        { value: 'watch', label: '🟢 保持观察' }
                                                    ].map(option => (
                                                        <button
                                                            key={option.value}
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                setBudgetAdviceFilter(option.value as any);
                                                            }}
                                                            className={`
                                                                px-3 py-1 text-xs font-bold rounded-md transition-all
                                                                ${budgetAdviceFilter === option.value
                                            ...
                                            <>
                                                <div className="overflow-x-auto">
                                                    <table className="w-full">
                                                        ...
                                                        <tbody>
                                                            {filteredBlResult.campaigns
                                                                .filter(campaign => {
                                                                    if (budgetAdviceFilter === 'all') return true;

                                                                    const benchmark = campaign.avgValue;
                                                                    if (!benchmark || benchmark <= 0) return true;

                                                                    const roi = campaign.actualValue;
                                                                    const threshold80 = benchmark * 0.8;

                                                                    if (budgetAdviceFilter === 'stop') {
                                                                        return roi < threshold80 && campaign.kpiType === 'ROI';
                                                                    } else if (budgetAdviceFilter === 'reduce') {
                                                                        return roi >= threshold80 && roi < benchmark && campaign.kpiType === 'ROI';
                                                                    } else if (budgetAdviceFilter === 'watch') {
                                                                        return roi >= benchmark || campaign.kpiType !== 'ROI';
                                                                    }
                                                                    return true;
                                                                })
                                                                .map((campaign, idx) => {
                                                ...
                                                {filteredBlResult && filteredBlResult.campaigns.length === 0 && (
                                                    <div className="flex flex-col items-center justify-center py-12 text-slate-400 border-t border-slate-100">
                                                        <div className="text-4xl mb-4">📭</div>
                                                        <div className="font-medium">当前筛选条件下暂无数据</div>
                                                        <div className="text-xs mt-2">请尝试切换其他筛选选项</div>
                                                    </div>
                                                )}
                                            </>
                                        )}
